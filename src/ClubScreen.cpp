...
lv_obj_t *ClubScreen::container = nullptr;
...
void ClubScreen::loadClubs()
{
    ...
    if (screen == nullptr)
    {
        Serial.printf("%s ERROR: screen is null\n", TAG);
        return;
    }

    if (container == NULL)
    {
        Serial.printf("%s ERROR: container is null\n", TAG);
        return;
    }

    Serial.printf("%s Clearing container\n", TAG);
    // Clear container
    lv_obj_clean(container);

    // Add club buttons
    Serial.printf("%s Adding %d club buttons\n", TAG, clubs.size());
    for (const Club &club : clubs)
    {
        lv_obj_t *btn = lv_btn_create(container);
        lv_obj_set_width(btn, lv_pct(100));
        lv_obj_set_height(btn, 28);  // Reduced height for compact display
        ...
        Serial.printf("%s  Added button for: %s\n", TAG, club.name.c_str());
    }
...
void ClubScreen::showError(const String &message)
{
    ...
    if (screen == nullptr || container == nullptr)
        return;

    lv_obj_clean(container);

    // Add error message
    lv_obj_t *errorLabel = lv_label_create(container);
    ...
    // Add retry button
    lv_obj_t *retryBtn = lv_btn_create(container);
